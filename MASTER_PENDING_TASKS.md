...
same for users and hosts" — then "go ahead, build the importer".

**Templates** — `docs/templates/Aajoo-Bulk-Import-Templates.xlsx` in the backend repo, generated by
`scripts/buildImportTemplates.py` **from `config/listingSchema.js`** (89 dropdown lists; nothing hand-typed). Properties = one row
across eight linked sheets (Properties 133 columns / Type_Details 69 / Amenities 23 / Rooms / Photos / Nearby_Places /
Rate_Periods / Cancellation_Tiers, joined by `property_ref`); Users and Hosts map 1:1 onto the admin "Add user" endpoint.
Deliberately absent: passwords, bank/UPI, KYC identity numbers and documents (the host enters those in the app; encrypted,
name-matched). Regenerate whenever the schema changes.

**Importer** — `scripts/importListingsWorkbook.js` (`--file <xlsx>` dry run; `--apply --as <admin email>`; `--only`, `--limit`,
`--check-urls`, `--keep-partial`, `--offline`). It drives the wizard's OWN handlers in-process — `saveStep1..4`, `uploadMedia`
(photos and the ownership document fetched from their URLs into Cloudinary, landscape/size rules applied), `saveStep5`,
`submitListing` as the admin acting for the host — and `adminUser.addUser` after its yup schema. So a bad row gets the
wizard's own refusal, and a listing that gets through is what a host would have typed; there is no second copy of any rule.
Dry run by default with one line per row and a CSV log; idempotent (same host + name + coordinates; same phone or email);
a half-built draft is removed when a later step refuses it. `submit_for_review = TRUE` needs what the app needs (verified
host or identity doc, ownership doc, photo minimums, Host Agreement current) — otherwise the row is left as a DRAFT with the
reason, never silently published. No password is read, written or printed; `send_invite = FALSE` means "Forgot password".

**Trial on dev:** one cottage under our host 194 → **#29313** (draft, 4 photos on Cloudinary) landed across 20 wizard tables
exactly as the app writes them; the planted bad row was refused for five reasons in one line; a re-run skipped #29313.
Found on the way: `saveStep3` stored a nearby place sent with a null lat/lng at **0,0** (`Number(null)` is 0) and marked it
Google — fixed, and the importer omits unknown coordinates. Test `theWorkbookImportsThroughTheWizard` (8 cases, builds
its own workbook); **186/186**. Commit `8833cda`.

**Open question for the client:** photos as URLs (what the template assumes) or as a folder of files — the importer takes
`photos/<property_ref>/` with a small change if it is a folder.
