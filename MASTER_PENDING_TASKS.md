...
| **3.6** | **Cloudinary — audited 2026-09-12; 2 exposures found, 1 fixed in code, 2 jobs left** | The account holds **1,003 assets**, not the 704 recorded in August. Measured with unauthenticated requests against real assets: **39 `property_documents` images answer HTTP 200** — ownership and identity papers photographed on a phone — and **12 `aajoo_reports` CSVs** do too. The raw PDFs beside them answer 401, which reads like protection and is not: this account blocks public PDF delivery by default, a free-plan setting that covers neither a JPEG of an Aadhaar nor a CSV. **Fixed in code:** documents upload as `authenticated` now and the reader signs by the type it finds in the stored URL. **Left to do: (a)** run `node scripts/privatiseDocumentAssets.js --apply` (dry-run by default; add `--reports-too` for the CSVs, which have probably been mailed somewhere) — it renames each asset to `authenticated` and rewrites `property_media` / `tbl_property_documents` in the same pass; **(b)** 228 unfoldered assets from Dec 2024 on, **46 of them A4-shaped portraits and 12 PDFs** — that is the drawer August's CV came out of, and it needs a human to look, not a script. | verified 09-12 — full Admin-API inventory + status probes per folder |
