...
| **3.6** | **Cloudinary — audited and migrated 2026-09-12; two things left** | The account holds **1,003 assets**, not the 704 recorded in August. Measured unauthenticated against real assets: **39 `property_documents` images answered HTTP 200** — ownership and identity papers photographed on a phone — and **12 `aajoo_reports` CSVs** still do. The 6 PDFs beside the images answered 401, which reads like protection and is not: this account blocks public PDF delivery by default, a free-plan setting. **Done:** documents upload `authenticated` now, the reader signs by the type it finds in the stored URL, and all **45** were migrated (`scripts/privatiseDocumentAssets.js --apply`) with our own rows repointed in the same pass — 0 left on `/image/upload/` or `/raw/upload/`, and 26 of the 39 turned out to be referenced by nothing at all. **Left: (a)** the exact VERSIONED old URL still answers 200 for any asset fetched before the rename, from Cloudinary's edge, `max-age` 2592000 — **thirty days, or one invalidation in the Media Library** (an API-side invalidate on the authenticated copy did not purge it); **(b)** 228 unfoldered assets from Dec 2024 on, **46 A4-shaped portraits and 12 PDFs** — the drawer August's CV came out of, needing a person to look rather than a script. The 12 report CSVs are behind `--reports-too` because a CSV has probably been mailed somewhere. | verified 09-12/13 — Admin-API inventory, per-folder status probes before and after, DB counts |
