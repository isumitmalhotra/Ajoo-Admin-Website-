...
| **3.6** | **Cloudinary — the unfoldered 228 have now been LOOKED AT, and it is worse than expected** | Contact-sheeted all 222 unfoldered images and read them. **22 are legible identity documents** — Aadhaar cards for several different people (numbers and QR codes readable), a college ID carrying name, father's name, DOB, address, phone and blood group, and an employee ID, several of them duplicated. **18 more are personal**: a UPI receipt naming both parties and their bank, a pathology report, a sign-in screen with the password field filled, a job application, a noticeboard of names and numbers, email signatures with direct lines, personal portraits. **11 are OURS and should never have been public**: rendered booking invoices carrying a guest's name, email and address, plus app screenshots with live booking and profile data. The remaining 171 are Cloudinary's own demo set, stock photography, our logo, scenery and code screenshots. **Only 15 of the 228 are referenced by any Aajoo table**, so 213 are orphans nothing on our side would miss. This is a **shared account**: most of the identity documents belong to somebody else's project, which is why deleting is the wrong first move and privatising is the right one — and why the real fix is Aajoo getting its own Cloudinary account. Classification and ids: `scratchpad/root_classified.json` (deliberately not committed — while those assets are still public, the id IS the key). **Needs a decision**, then one script run. | verified 09-13 — six contact sheets read by eye; DB cross-reference across property_media, tbl_property_documents, tbl_attachments |
