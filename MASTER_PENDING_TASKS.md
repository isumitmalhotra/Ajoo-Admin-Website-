...
| **3.13** | **BotPenguin is handed the user's session JWT** | `botpenguinWidget.ts` passes the signed-in user's website bearer token to the vendor's script as a `ctx-token` attribute so the bot can start an authenticated support session. A third party holding live account tokens is a larger exposure than any cookie: replace it with a short-lived, single-purpose token minted for `/bp/session/start`, and stop passing email/phone as script attributes. Needs a decision on the bot flow, so listed rather than done. | code — `src/services/botpenguinWidget.ts` |
