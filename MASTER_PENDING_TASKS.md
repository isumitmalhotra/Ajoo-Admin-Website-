...
| **3.6** | **Cloudinary — audited and migrated 2026-09-12/13; one job left** | The account holds **1,003 assets**, not the 704 on record. Measured unauthenticated against real assets, **51 answered HTTP 200 that should not have**: 39 `property_documents` images (ownership and identity papers photographed on a phone) and 12 `aajoo_reports` CSVs — and those CSVs carry **guest_name and guest_phone**, are generated by the chatbot on request, and had their raw public URL pasted straight into a chat message. The 6 PDFs beside the images answered 401, which reads like protection and is not: this account blocks public PDF delivery by default, a free-plan setting. **Done:** both uploaders store `authenticated` now (documents, and the report generator, which also hands out a signed link that expires in 24 hours and says so); the reader signs by the type it finds in the stored URL; and all **60** assets were migrated with our own rows repointed in the same pass. The account's document folders now read 39 + 6 + 12 + 3, **every one authenticated, none public**; 26 of the 39 turned out to be referenced by nothing at all. Every report CSV's old URL answers 404. **Left: (a)** the document IMAGES keep a cache residue — the exact versioned old URL still answers 200 from Cloudinary's edge for any asset fetched before the rename, `max-age` 2592000, so **thirty days or one invalidation in the Media Library** (an API-side invalidate did not purge it); **(b)** 228 unfoldered assets from Dec 2024 on, **46 A4-shaped portraits and 12 PDFs** — the drawer August's CV came out of, needing a person to look rather than a script. | verified 09-12/13 — Admin-API inventory before and after, per-asset status probes, DB counts |
