...
| 3b | Explore — home chrome | `screens_renter/home/homescreen.dart` (+ components) | 🟡 | **Cards ✅ + search pill ✅** (scaffold search-card: pin + text + teal search button; `onTap` → search sheet wired). Map + `POST /properties/search` + draggable listings all ✅ working. Remaining: branded header, category circles, trust bar. Map-first structure kept (Google Maps wired). |
