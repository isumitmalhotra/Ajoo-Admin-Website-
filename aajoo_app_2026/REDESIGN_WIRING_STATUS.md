...
| H-2 | Host Bookings | `screens_host/booking_history/booking_history_screen.dart` | ✅ | Re-skinned to scaffold `host_bookings` — teal app bar + 4 status tabs (Upcoming/Ongoing/Completed/Cancelled) bucketing the real `HostBookingHistory` data; cleaner white cards w/ guest avatar + status-colored badge + price. `HostBookingHistoryController` (getHostBookingHistory / submitReview / submit overlay) + review dialog kept. |
| H-3 | Host Support | `screens_host/support/host_support_screen.dart` | ✅ | Already on-brand + fully wired (dedicated support tel, WhatsApp, **BotPenguin chat webview**, email, emergency, portal link, host FAQ). No re-skin needed — teal via P0. Mapped to the shell's 4th tab + drawer. |
| H-4 | Host Profile | `screens_host/profile/host_profile.dart` | 🟡 | Polished (name + Host pill header, teal camera button). Full KYC/properties/edit account screen (798 lines) — kept functional, teal via P0; deeper redesign optional (like guest profile). |
| H-5 | Add Property | `screens_host/add_property/host_property_listing_screen.dart` | ✅ | **Redesigned to the scaffold `host_add_property` stepped wizard** — white header + segmented **progress bar** + "Step X of 6" label + sticky footer (Continue / Submit). 6 steps: Property Type · Location · Photos · Pricing · Preferences · Publish. Implemented as **one Form + IndexedStack** (all steps stay mounted) so every field/validator/controller and the entire `_saveProperty` mapping (~50 fields, PG/Party sub-forms, amenities/tags, image + 3-doc upload) are **unchanged and fully wired**. Publish-time validation now jumps to the first incomplete step. |
| H-6 | Payout | `screens_host/payout/payout_page.dart` | ✅ | Already on the new design + fully wired — real earnings (`PlanOverviewCard`), bank-account card (add/edit → `AddPayoutAccountPage`), request-payout bottom sheet (`createPayoutRequest`), payout history. `PayoutController` → `getPayoutList` / `getHostAccountDetails` / `saveHostAccountDetails` / `createPayoutRequest`. Left as-is (no churn). |
| H-7 | Invoices | `screens_host/invoices/invoice_page.dart` | ✅ | **Was hardcoded mock invoices — now wired to real host transactions** (`getHostTransactions` → `transactionHistoryResponse`). Each payment = an invoice card (invoice #, property, guest, amount, date, paid/pending badge); share + download build the PDF from **real** transaction data. Re-skinned to white/teal cards + empty/loading states. |
| — | Add-Payout · Update-Property · View-Host-Property | (existing wired screens) | 🟡 | Secondary screens reached via payout/profile. Wired + teal via P0; not individually redesigned. |
