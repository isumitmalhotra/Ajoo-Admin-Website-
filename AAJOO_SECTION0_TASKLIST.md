...
| **A8** | **CMS-driven content** | Every public page's content editable via Admin (per-page CMS field lists given in spec). No hardcoded marketing copy. | 🟡 **Home, About, Contact live 2026-08-07** (`tbl_cms_content` + `/admin/cms/content/*` + public `/public/cms/:page`; editor at `/redesign/admin/cms`). Getting Started + FAQ still to wire. |
...
| **A10** | **SEO readiness** | Per-page dynamic `<title>`/meta description, Open Graph tags, schema markup, clean URLs. Copy for titles/meta/OG is provided per page below. | 🟡 per-page title/description/OG ship on 7 redesign pages; **JSON-LD schema + canonical URLs still missing** |
...
| A8 | CMS-driven content | 🟡 | **Working for Home, About and Contact.** The old CMS was unusable — `tbl_cms_pages` does not exist in the DB (faq-page/tc-page endpoints return "table doesn't exist") and the admin screen's edit/view/New buttons did nothing. Replaced with a page/key/value store; one schema (`lib/cmsSchema.ts`) drives both the editor and the pages, and a missing row means "use the code default" — so an empty CMS renders the shipped site. Getting Started + FAQ not yet wired. |
...
| A10 | SEO (meta/OG/schema) | 🟡 | `useDocumentMeta` sets title, description and OG/Twitter tags on **7 redesign pages**, with separate OG copy where the spec provides it. The July "❌ no per-page meta" finding is out of date. **Still missing: schema.org JSON-LD and canonical URLs** (verified 2026-08-07: 0 occurrences of either). |
