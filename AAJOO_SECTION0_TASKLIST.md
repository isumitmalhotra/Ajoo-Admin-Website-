...
| **A8** | **CMS-driven content** | Every public page’s content editable via Admin (per-page CMS field lists given in spec). No hardcoded marketing copy. | ✅ **all 5 pages live 2026-08-07** — Home, About, Contact, Getting Started, Help Center. 128 fields, editor at `/redesign/admin/cms`. |
...
| **A10** | **SEO readiness** | Per-page dynamic `<title>`/meta description, Open Graph tags, schema markup, clean URLs. Copy for titles/meta/OG is provided per page below. | ✅ shipped 2026-08-07 — titles/meta/OG + canonicals + Organization, WebSite, FAQPage and LodgingBusiness JSON-LD |
...
| A8 | CMS-driven content | ✅ | Page/key/value store (`tbl_cms_content`) + `/admin/cms/content/*` + public `/public/cms/:page`. One schema (`lib/cmsSchema.ts`) drives the editor and the pages; a missing row means "use the code default", so an empty CMS renders the shipped site and clearing a field is the undo. The old CMS was unusable — `tbl_cms_pages` does not exist in the DB and the admin screen's buttons did nothing. |
...
| A10 | SEO (meta/OG/schema) | ✅ | `useDocumentMeta` sets title/description/OG on 7 pages **and now canonical + og:url** (campaign params stripped; query kept, so `/property?id=8` and `?id=9` stay distinct). Organization + WebSite JSON-LD are static in `index.html` so a crawler that does not run our JS still sees them; FAQPage is built from the live corpus; PropertyDetail — which previously had **no title, meta or schema at all** — now emits LodgingBusiness, gated on the listing having loaded and with `aggregateRating` only when real reviews exist. |
...
| Page | Status | Finding |
|------|--------|---------|
| **B1 Getting Started** | 🟡 | The July "no page, no route" finding is out of date — `/getting-started` exists (`redesign/pages/GettingStarted.tsx`) and `IntentGate` handles first-visit routing on `/`. GS-5 hero slider and GS-7 scroll-reveal nav are shipped. **Remaining:** the spec’s exact copy and section list (badge, headline "Stay Better. Host Smarter. Belong Everywhere.", 6 cards, How It Works, Our Promise, Why We Exist, bottom CTA) plus its explicit REMOVE list, and CMS wiring. |
| **B2 Home / Explore** | ✅ | Rebuilt 2026-08-07 to CNT-2: all **15 sections in spec order** with spec copy, on `/explore` (`redesign/pages/Explore.tsx`). SEO title/meta + separate OG copy wired. Newsletter posts to a real endpoint (`/newsletter/subscribe`); Guest Reviews reads `/public/reviews/recent` and shows an honest empty state — `tbl_reviews` has 0 rows. **Remaining:** the Play Store link is disabled until the Android app is published. |
| **B3 About Us** | ✅ | Rebuilt 2026-08-07 to CNT-3 on `/about`: hero, Our Story, Vision, Mission, the five named Values, six What-Makes-Aajoo-Different cards, Looking Ahead and the closing passage — copy verbatim. Mission/Vision carry the A6 illustrations. Spec supplies no SEO block for this page, so the existing title/meta stand. ✅ **CMS-editable** (A8). |
