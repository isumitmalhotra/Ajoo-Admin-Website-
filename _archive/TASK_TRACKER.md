...
> **Tracker Updated:** 4 Jun 2026

---

## ⚠️ DEV-BYPASS — Active Temporary Changes (Must Revert Before Production)

> Added **4 Jun 2026** for local testing. All bypasses are marked with `// [DEV-BYPASS]` in code.

| # | File | Change | How to Revert |
|---|------|---------|---------------|
| 1 | `aajooBackend-2026/controllers/user.controller.js` ~line 158 | Removed mandatory `req.file` check in `createUser` — signup no longer requires an ID document | Restore: `if (!req.file) { return common.response(req, res, commonConfig.errorStatus, false, "Document image is required"); }` |
| 2 | `aajooBackend-2026/controllers/user.controller.js` ~line 211 | KYC doc record creation is now conditional on `req.file && doc_type && doc_number` — previously always created | Restore original unconditional `model.user_kyc_docs.create(...)` block |
| 3 | `aajooBackend-2026/controllers/user.controller.js` ~line 255 | OTP `"000000"` always passes verification (dev master OTP) — bypasses DB OTP lookup entirely | Remove the `if (reqData.otp === '000000') { ... }` block before the normal OTP validation |
| 4 | `aajoo_app_2026/lib/ui/screens_common/auth/basic_info/basic_info_screen.dart` ~line 230 | `_validateDocument()` returns `null` immediately when no doc selected — was `'Please upload a document'` | Restore: `if (selectedDocument == null) { return 'Please upload a document'; }` |
| 5 | `aajoo_app_2026/lib/ui/screens_common/auth/auth_controller.dart` ~line 208 | `idDoc: governmentIdImage.value` (nullable) — was `governmentIdImage.value!` (non-nullable bang) | Restore: `idDoc: governmentIdImage.value!` |
| 6 | `aajoo_app_2026/lib/ui/screens_common/auth/auth_controller.dart` ~line 209 | `docType: int.tryParse(...) ?? 0` — was `int.parse(...)` which throws on empty string | Restore: `docType: int.parse(signupData['doc_type'].toString())` |
| 7 | `aajoo_app_2026/lib/ui/screens_common/auth/basic_info/basic_info_screen.dart` | Added `_skipStep3AndSave()` method and **"Skip for now (dev)"** `TextButton` below stepper controls on Step 3 | Remove the entire `_skipStep3AndSave()` method and the `if (currentStep == 2)` skip button block in `_buildStepperControls` |
| 8 | `aajooBackend-2026/schema/user.schema.js` lines 56-77 | `doc_type` and `doc_number` made optional (`.optional().nullable()`) in `createUser` Yup schema — was `.required()` | Restore `doc_type: yup.number().required('document type is required')` and `doc_number: yup.string().required("Document number is required")` |
| 9 | `aajoo_app_2026/lib/controller/auth_controller.dart` ~line 267 | Fixed bang `governmentIdImage.value!` → `.value`, `int.parse` → `int.tryParse(...) ?? 0`, added `?? ''` on docNumber | Restore: `idDoc: governmentIdImage.value!`, `docType: int.parse(signupData['doc_type'].toString())`, `docNumber: signupData['doc_number']` |

> **Pre-production checklist:** Add Cloudinary credentials (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`) to Render env vars → revert all 5 items above → redeploy backend.
...
**Scope note (May 27 2026):** For current delivery, ignore mobile-only code and focus on website frontend + shared backend. Mobile frontend will be added later in a separate folder. Primary mobile folder to exclude from scope: `aajoo_homes-main/`.
...
## 4.1) Backend-Frontend Connection Audit (May 27 2026)

This section lists the concrete wiring gaps found between frontend calls and backend routes. Each item is actionable and scoped as a task.

| ID | Task | Status | Owner | Dependency |
|---|---|---|---|---|
| INT-01 | Unify API base URL usage so all client calls use a single `API_BASE_URL` source (remove hard-coded render/local URLs). | ⬜ | FE | None |
| INT-02 | Update admin auth verification to use a real backend endpoint (add `/admin/verify-token` or change frontend to an existing verify route). | 🔄 | FE/BE | Resolution: add `GET /admin/verify-token` (locked in `API_CONTRACT_HANDOFF.md` § 10). Impl pending A-08 / B-08. |
| INT-03 | Implement or map missing finance endpoints under `/admin/finance/*` (ledger, payout, invoice, reconciliation, reports). | 🔄 | BE | Contract locked (`API_CONTRACT_HANDOFF.md` §§ 2-7). Impl pending A-03..A-06. |
| INT-04 | Align host bookings endpoints (frontend expects `/host/bookings/*`; backend currently exposes `/host/booking-history`). | 🔄 | FE/BE | Resolution: add new path alongside old; old deleted Day 10. Impl pending A-07. |
| INT-05 | Align host payout account endpoints (frontend expects `/host/payout-account/*`; backend exposes `/payout/account/*`). | 🔄 | FE/BE | Resolution: add `/host/payout-account/{get,update}` alongside existing. Impl pending A-07. |
| INT-06 | Align host payout history endpoints (frontend uses `/host/payout/history`; backend uses `/payout/history`). | 🔄 | FE/BE | Resolution: add new path. Impl pending A-07. |
| INT-07 | Implement host support ticket endpoints for search/create/reply to match frontend paths. | 🔄 | BE | Contract locked. Impl pending A-07. |
| INT-08 | Align host messaging endpoints (frontend uses `/host/messages/list` and `/host/messages/send`; backend uses threads/conversation). | 🔄 | FE/BE | **Resolution: CUT REST scope** — `HostCommunication.tsx` consumes tickets API (`category="GENERAL"`) instead. Socket.io chat upgrade deferred post-sprint. Account B updates FE in B-04. |
| INT-09 | Implement host performance summary endpoint or remove the frontend link to it. | 🔄 | FE/BE | Resolution: ONE endpoint `GET /host/performance/summary` (not 4). Impl pending A-07. |
| INT-10 | Add `/host/onboarding/submit` backend handler or update frontend to the correct host onboarding endpoint. | 🔄 | FE/BE | Contract locked. Impl pending A-07. |
| INT-11 | Add admin host KYC approve/reject endpoints or map frontend actions to existing backend routes. | 🔄 | FE/BE | Contract locked (`API_CONTRACT_HANDOFF.md` § 9). Impl pending A-08. |
| INT-12 | Fix password-reset flow so `/user/update/forget-password` works for logged-out users (remove auth requirement or add token strategy). | 🔄 | BE | Resolution: add new public `POST /user/forgot-password/request` (`API_CONTRACT_HANDOFF.md` § 13). |
| INT-13 | Verify CORS `FRONTEND_URL` config matches the deployed admin domain; update environment defaults if needed. | ⬜ | BE/Ops | BLK-01 |
...
| GAP-08 | Wire notification center to backend stream/feed | ✅ | FE | B-09: real search + 30s poll + PUT read (A-14) |
...
| P2-API-02 | Freeze payload contracts for host endpoints | ✅ | BE | Done 2026-06-09 in `API_CONTRACT_HANDOFF.md` § 8 |
| P2-API-03 | Add versioning/error envelope conventions | ✅ | BE | Done 2026-06-09 in `API_CONTRACT_HANDOFF.md` § 0.1 |
| P2-API-04 | Add pagination/sort/filter consistency across all lists | ✅ | BE | Done 2026-06-09 in `API_CONTRACT_HANDOFF.md` § 0.6 |
| P2-API-05 | Add webhook/event contracts for real-time updates | ✅ | BE | Done 2026-06-09 in `API_CONTRACT_HANDOFF.md` § 11 (Didit webhook); socket.io upgrade deferred post-sprint |
...
| P3-ADM-05 | Admin host management: performance + payout panes | 🔄 | FE | FE done (B-07): 4-tab dialog + perf/payout slices, mock-stub auto-flips to real on A-08 |
...
| P3-GST-05 | Audit mobile responsiveness of high-traffic guest pages — out of scope (current phase) | 🔄 | FE | regression pass pending |
...
| P3-HST-06 | Host statements API integration (currently mock table) | ✅ | FE | POST /host/statements/search + normalizer (B-08) |
| P3-HST-07 | Host support ticketing integration (currently local mock) | ✅ | FE | search+create wired to /host/support/tickets/* (B-08) |
| P3-HST-08 | Host communication center integration (currently local mock) | 🔄 | FE | no backend — A-07 cut /host/messages/* (INT-08); mock-only, honest empty when real |
| P3-HST-09 | Host performance analytics integration (currently mock trends) | ✅ | FE | GET /host/performance/summary single-window projection (B-08) |
| P3-HST-10 | Host role-safe guard + unauthorized redirects | ✅ | FE | B-12: HostRoute by JWT role + host→/host/dashboard bounce |
