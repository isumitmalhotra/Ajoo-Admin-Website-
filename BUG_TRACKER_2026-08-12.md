...
| **E-14** | Backend deploy pending: `PAGES` whitelist now includes `safety` (commit `16307f8`). Until Render redeploys, `public/cms/safety` 404s and both platforms render shipped defaults — correct, but not yet editable from admin. | ✅ Deployed 2026-08-13 |
| **E-17** | ✅ **Listing submit worked end to end on 2026-08-13**, after two blocking bugs were fixed (commit `6e0a65a`). Property 29252 was created via the app with the host test account, the app showed "Listing submitted — pending admin approval", and the row plus its 3 document rows and category link were deleted afterwards; catalogue back to 29,241. **Upload size matters:** ~4MB of photos never completed (10 min, no error, before the timeout fix); ~180KB completed in about 90 seconds. Worth considering client-side image compression before a host on mobile data tries this. | Done |
| **E-16** | 🟡 **The host listing form offers categories guests cannot browse.** Found by walking the Add Property flow on device 2026-08-13. Step 1's Property Type chips include **Resort, Couple, Party** — the three in `kHiddenBrowseCategories`, filtered out of Browse by Category on both the home and pre-booking screens — plus **Family**, which is not in the guest list at all. So a host can file a listing under a category no guest can ever browse to, which is how those 10 orphaned listings happened in the first place. Either drop them from the host form or unhide them for guests; having both is the bug. | **Needs a decision** |
| **E-15** | 🔴 **Category filters return nothing because almost no property has a category.** Reported as "filter by Flats/Apartments shows nothing". The filter code is correct end to end — the app sends `cat_id`, and `property.controller` matches it against `tbl_prop_to_cats`. The data is the problem: **only 16 of 29,241 live properties carry any category link**, and all 16 are hand-made test/demo listings (`Test property`, `EDIT REPRO`, `E2E Wizard Test`, `QA End ToEnd Villa`…) clustered in Gurugram/Noida/Delhi/Mohali/Kasauli/Chennai. Counts: Apartments 7, Villas 4, Cottages 1, Heritage 1, Pet-Friendly 1, **Homestays 0, Farm Stays 0, Boutique 0, Luxury 0**. The ~28,918 CSV-imported listings have no category rows, and nothing to derive them from: `property_type` is null on 29,224 rows, `category_tier` null on 29,222, and only **0.6%** (178) have a recognisable type word in the name. **RESOLVED 2026-08-13.** The client supplied the source CSV — it has **no type column** (name, lat/long, desc, price, city, zip, state, country, contact, email, smoking, check-in/out), confirming nothing was derivable. On their instruction, all 29,225 uncategorised listings were split evenly across the nine browse categories (~3,247 each) via `scripts/assign_property_categories.js` on the backend. **This is a deliberate placeholder, not real classification** — rollback SQL is in the script header. Left alone: the 16 real tagged listings, the hidden `couple`/`party`/`Resort` (assigning there would make listings invisible), and `is_luxury` (what LUX actually filters on). Verified on production: every category returns results near Chandigarh. The app also keeps the honest empty state for genuinely empty filters. | ✅ Done |
